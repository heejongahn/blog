...
웹 개발자로 일하다 보면 CSS의 세계는 참 무궁무진하다는 사실을 느낄 일이 많다. 자주 다루는 몇 가지 속성만으로도 대부분의 용례가 충분히 커버 가능하지만, 오히려 그래서인지 여러 흔치 않은 용례를 고려하여 만들어진 속성들을 맞닥뜨릴때면 더더욱 즐거운 마음으로 놀라곤 한다.

이 글에서는 잘 알려지지 않은, 하지만 알아두면 분명 유용하게 써먹을 일이 생길 CSS 속성들에 대해 소개한다. 하나씩 찾아보면 사실 이미 잘 정리된 자료들이 많은 만큼, 해당 속성에 관한 모든 세부 내용을 최대한 세세히 다루기보단 이런 속성이 존재하며 대략 어떤 용도로 사용된다는 점 정도를 소개하는 데에 초점을 맞추려 애썼다.
