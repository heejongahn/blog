...
4. Nuxt.js 를 한 번 빌드한 뒤 GitHub에 푸시한다
...
추가적으로, 이미 시간이 없어 미루어뒀지만 몇 달 간 쌓아둔 블로그 개선 방안이 몇 개 있었다. 꽤나 간단한 사이트인만큼, 해당 작업을 전부 다 하는거랑 새로 만드는게 그렇게까지 들어가는 노력이 많이 차이나지 않을 것이란 판단이 들어 새로 만들기로 결정했다. 사실 그런 이유들과 별개로 그냥 다시 만들고 싶었던 것도 있다.
...
![기존 사이트의 것을 거의 그대로 가져온 소개 페이지](/assets/remaking_blog_about_after.png)
...
다음으론 OpenGraph, Twitter 등 소셜 미디어 공유에 사용되는 메타 태그를 추가하고 Google Analytics 를 셋업하는 등의 눈에 보이지 않는 작업을 진행했다. 메타 태그 셋업에는\[react-helmet](https://github.com/nfl/react-helmet)을 사용했고, GA는 \[공식 플러그인](https://github.com/gatsbyjs/gatsby/tree/master/packages/gatsby-plugin-google-analytics)이 존재해서 쉽게 붙일 수 있었다.

\## 기술 선택

이번에 블로그를 만들면서 사용한 기술들이다. 각 분야 최고의 툴을 찾기 위해 오래 리서치하기보단 빠르게 익혀 적당히 쓸만한 결과를 만들 수 있는 툴이라는 생각이 들면 바로 도입하는 식으로 작업했다.
...
정확한 워딩은 기억이 안 나는데, 얼마 전에 재미도 한정된 자원이고 소비적인 활동에서 너무 많은 재미를 느끼면 생산적인 일을 할 동기를 잃는다는 내용의 트윗을 보고 공감했다. 새로운 기술로 생산적인 일을 하는 재미가 있으니 확실히 유튜브에 낭비하는 시간이 줄어들었다. 일찍 출근해서 글쓰기나 블로그 개선에 시간을 쓰는 습관은 계속 가져가려 한다.
